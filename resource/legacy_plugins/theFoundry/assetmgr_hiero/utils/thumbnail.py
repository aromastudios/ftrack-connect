...
def setupThumbnails(items, context):
  ...
  with ScopedProgressManager(len(items)) as progress:
    ...
    # As items may be of mixed type (say shot + image registrations) then make
    # sure that we check the thumbnail options for all item types
    thumbOpts= {}

    for i in items:
      with progress.step("Preparing thumbnail for %s" % i.getString()):
