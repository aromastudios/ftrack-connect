...
class PublishShotClipsDialog(QtWidgets.QDialog):
  """
...
class PublishShotClipsWidget(QtWidgets.QWidget):
  """
  ...
  def _buildUI(self, layout):
    ...
    parentPickerLayout = QtWidgets.QHBoxLayout()
    parentPickerLayout.addWidget(QtWidgets.QLabel("Look for matching Shots under:"))
  ...
  def _buildClipsTab(self):
    ...
    mediaWidget = QtWidgets.QWidget()
    mediaWidget.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
    mediaWidgetLayout = QtWidgets.QVBoxLayout()
    mediaWidgetLayout.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
    mediaWidget.setLayout(mediaWidgetLayout)

    # - Shared Media

    self._sharedClipsGroup = QtWidgets.QGroupBox(l("Some Source Clips are Shared "+
      "and used in more than one Shot in the Edit"))
    mediaWidgetLayout.addWidget(self._sharedClipsGroup)
    sharedClipsGroupLayout = QtWidgets.QVBoxLayout()
    self._sharedClipsGroup.setLayout(sharedClipsGroupLayout)

    self._sharedIgnoredRadio = QtWidgets.QRadioButton(l("Don't {publish}"))
    self._sharedToSequenceRadio = QtWidgets.QRadioButton(l("{publish} at the level above the Shots"))
    self._sharedToCustomRadio = QtWidgets.QRadioButton(l("{publish} to another location"))
    ...
    self._previewWidget.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
    mediaWidgetLayout.addWidget(self._previewWidget)

    # - Options

    self._clipOptionsGroup = QtWidgets.QGroupBox(l("Options"))
    optionsGroupLayout = QtWidgets.QVBoxLayout()
    ...
    hieroOptionsGrid = QtWidgets.QGridLayout()

    ## @todo we should have some base widget for this

    hieroOptionsGrid.addWidget(QtWidgets.QLabel(l("{asset} name:")), 0, 0)

    self._clipNameCombo = QtWidgets.QComboBox()
    self._clipNameCombo.addItems(("Clip Name", "Custom"))
    hieroOptionsGrid.addWidget(self._clipNameCombo, 0, 1)

    self._clipNameCustomField = QtWidgets.QLineEdit()
    hieroOptionsGrid.addWidget(self._clipNameCustomField, 0, 2)

    self._replaceClipSource = QtWidgets.QCheckBox(l("Link Source Clips to "+
        "{published} {assets}"))
    hieroOptionsGrid.addWidget(self._replaceClipSource, 1, 1, 1, 2)

    self._ignorePublishedClips = QtWidgets.QCheckBox(l("Ignore Source Clips that are "+
      "already {published}"))
    ...
    #column really wide
    hieroOptionsHBox = QtWidgets.QHBoxLayout()
